<template>
    <div class="w-full flex flex-col items-center p-2 pt-0">
        <div
            ...
            <div class="my-4">
                <el-button size="large" :type="isFollow ? 'danger' : 'primary'" @click="followOne"
                    :disabled="!id || id == store.userId">{{ isFollow ? '取消关注' : '现在关注' }}</el-button>
            </div>
        </div>
        <div class="w-full button">
            ...
            <el-button class="w-full" :class="path == '/blog/kind' ? 'active' : ''"
                @click="router.push(id ? `/blog/kind?id=${id}` : '/blog/kind')"><el-icon :size="18" class="mr-4">
                    <Tickets />
                </el-icon>文章归档</el-button>
            <el-button class="w-full" :class="path == '/blog/friend' ? 'active' : ''"
                @click="router.push(id ? `/blog/friend?id=${id}` : '/blog/friend')"><el-icon :size="18" class="mr-4">
                    <PhoneFilled />
                </el-icon>关注列表</el-button>
            <el-button class="w-full" :class="path == '/blog/board' ? 'active' : ''"
                @click="router.push(id ? `/blog/board?id=${id}` : '/blog/board')"><el-icon :size="18" class="mr-4">
                    <DataBoard />
                </el-icon>留言板</el-button>
        </div>
        <div class="w-full min-h-200 mt-8 bg-white/80 shadow-lg rounded-md pt-2 ">
            <el-check-tag size="large" class="m-2 cursor-pointer min-w-[65px] text-center" effect="plain"
                v-for="tag in tags" :key="tag" :checked="store.currentTag == tag"
                @change="currentTag = currentTag != tag ? tag : ''" :style="tagStyles[tag]">{{ '#' + tag }}</el-check-tag>
        </div>
        <div
            class="w-full min-h-200 my-8 bg-white/80 bg-opacity-80 p-4 text-lg text-gray-600 shadow-lg rounded-md overflow-auto">
            ...
            <div class="my-4 cursor-pointer hover:bg-slate-200 flex justify-between items-center px-4"
                :class="currentType == item.type ? 'bg-slate-300' : ''" v-for="item in store.type" :key="item.type"
                @click="currentType = currentType != item.type ? item.type : ''">
                {{ item.type }}
                <span
...
<script setup>
import { getAllFollowees } from '../http/api'
