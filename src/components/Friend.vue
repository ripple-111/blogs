            ...
            <el-card v-for="item in followers" :key="item.id">
                <template #header>
                    <div class="flex justify-between items-center">
                        ...
                        <el-button type="primary" :icon="View" @click="router.replace(`/blog?id=${item.id}`)"></el-button>
                    </div>
                </template>
                <div class="font-semibold flex justify-between items-center">
                    最新博客
                    <el-button type="primary" @click="cancel(item.id)" v-if="!id">取消关注</el-button>
                </div>
                <div class="px-1 mt-2 flex border-t-2 border-slate-300 bg-blue-200 hover:cursor-pointer"
                    v-for="i in item.userArticles" :key="i.id">
                    <div class="cursor-pointertext-lg font-semibold w-3/4 text-ellipsis overflow-hidden whitespace-nowrap h-10 leading-[40px]"
                        @click="router.push(`/display?id=${i.id}`)">{{ i.title }}</div>
                    <div class="w-1/4 overflow-hidden whitespace-nowrap text-sm leading-[40px]">{{ interval(i.time) }}
                    </div>
                </div>
            </el-card>
